...
Warnings:
- Experimental and resource intensive; quality is not guaranteed on small VRAM GPUs.
- Requires CUDA-enabled PyTorch and coqui-ai/TTS compatible versions.
- Expects dataset files under data/model_input/ with metadata file Dataset_clips.txt.
- Writes checkpoints and logs under data/model_output/finetune-run-YYYYMMDD_HHMMSS.
- Ensure audio settings and text processing match the pre-trained model to reduce instability.
'''
...
if __name__ == "__main__":
    # CUDA memory tuning for fragmentation on small VRAM
    ...
    BASE_OUTPUT = DATA_DIR / "model_output"        # folder to store runs
    ...
    # Main VITS config (kept as provided)
    config = VitsConfig(
        text_cleaner=None,
        use_phonemes=False,
    ...
    # Load samples with custom formatter
    print("\n📂 Loading dataset...")
